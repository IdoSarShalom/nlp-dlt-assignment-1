...
The GRU model converged faster (epoch 10-11) with stable loss reduction. The BiLSTM model required more epochs and higher capacity to reach comparable performance.
...
\section{Conclusion}
Based on the comparative analysis, the \textbf{GRU model with Word2Vec embeddings} is the optimal configuration.

\paragraph{Justification:}
\begin{enumerate}
    \item \textbf{Superior Performance:} Higher validation accuracy (92.80\%) and lower loss (0.1344).
    \item \textbf{Efficiency:} Required fewer units (96 vs 192), resulting in a lighter model with faster training.
    \item \textbf{Embedding Quality:} 300d Word2Vec embeddings provided richer semantic features than 200d GloVe.
\end{enumerate}

The findings demonstrate that the simpler, more efficient GRU architecture, paired with high-dimensional pre-trained embeddings, outperforms the more complex BiLSTM model for this task.
